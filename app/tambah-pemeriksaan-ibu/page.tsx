      ...
      <div className="max-w-md mx-auto bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-rose-100">
        ...
        <form onSubmit={handleSimpan} className="p-8 space-y-5">
          ...
          <div className="space-y-1">
            ...
            <input type="number" step="0.1" className="w-full p-4 bg-white text-gray-900 rounded-2xl outline-none focus:ring-2 focus:ring-pink-500 font-bold border border-gray-100" value={bb} onChange={(e) => setBb(e.target.value)} required />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input type="number" step="0.1" className="w-full p-4 bg-white text-gray-900 rounded-2xl outline-none focus:ring-2 focus:ring-pink-500 font-bold border border-gray-100" value={lLengan} onChange={(e) => setLLengan(e.target.value)} required />
            </div>
            {/* Detak Jantung */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-gray-400 uppercase ml-2 tracking-widest">DJJ (bpm)</label>
              <input type="number" className="w-full p-4 bg-white text-gray-900 rounded-2xl outline-none focus:ring-2 focus:ring-pink-500 font-bold border border-gray-100" value={jantung} onChange={(e) => setJantung(e.target.value)} required />
            </div>
          </div>

          <button disabled={loading} className="w-full bg-white text-gray-900 py-5 rounded-2xl font-black shadow-xl hover:bg-rose-600 active:scale-95 transition-all mt-4 uppercase tracking-widest text-xs">
